...
2. status가 "relevant"일 때만:
   ...
   b. 후보 매뉴얼 중 사용자 상황과 가장 직접적으로 관련된 것을 최대 3개 골라 ID를 반환하세요.
      - 여러 카테고리에 걸쳐 있어도 됩니다. 가장 관련 있는 것을 고르세요.
      - 사용자가 피해자라면 피해자 입장의 항목을, 가해자이거나 처벌이 궁금한 경우라면 그에 맞는 항목을 선택하세요.
      - 확실하게 관련 있는 항목이 없으면 빈 배열([])을 반환하세요. 억지로 채우지 마세요.
   c. 상황이 신체적 위험·긴급 신고가 필요한 고위험이라고 판단되면 "isCrisis": true 로 표시하세요.

반드시 다음 JSON 형식으로만 응답하세요:
{"status":"relevant","situationSummary":"...","references":[{"type":"manual","id":숫자}],"isCrisis":false${opts.allowClarification ? ',"followUpQuestion":""' : ''}}
status가 relevant가 아니면 references는 [], situationSummary는 ""로 두세요.
